...
export default function AuthForm(props) {
  ...
  const usernameLengthNot = action === 'sign-up'
    ? 'd-flex justify-content-center mt-2'
    : 'hidden';
  const usernameLengthNotClass = username.length < 9 && username.length > 0
    ? 'green'
    : 'red';
  let submitButtonRes;
  if (action === 'sign-in') {
  ...
  return (
    <div className="bg-white p-4 rounded shadow mt-4">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={usernameLengthNot}>
          <p className={usernameLengthNotClass}>Username must be maximum 9 characters&nbsp;<FontAwesomeIcon icon={username.length < 9 && username.length > 0 ? faCheck : faX} /></p>
        </div>
      </form>
    </div>
  );
